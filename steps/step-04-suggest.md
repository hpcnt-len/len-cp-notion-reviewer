...
**⚠️ 목차(TOC) 추가 제안 시:**
TOC 블록은 Notion API로 직접 생성할 수 없으므로 다음과 같이 표시:

```
**🟡 Medium - 개선 권장 항목**

**1. 목차(Table of Contents) 추가 필요** ⚠️ 수동 추가
- 현재 상태: 문서 길이가 길지만 목차가 없음
- 개선안: 문서 최상단에 목차 추가 (Notion에서 `/toc` 입력)
- ⚠️ 참고: TOC는 API로 추가 불가, step-06에서 수동 추가 안내 제공
```
...
문서가 너무 긴 경우 분리를 제안합니다:

```
**📂 문서 분리 제안** ⚠️ 신규 문서 생성 필요

이 문서는 다음 이유로 분리가 권장됩니다:
- [물리적 길이 / 주제 분산 / 구조 과도] 중 해당 사항

**분리 계획:**

| # | 신규 문서 제목 | 이동할 내용 | 현재 위치 |
|---|--------------|------------|----------|
| 1 | "[제안 제목 1]" | [내용 요약] | [섹션명] |
| 2 | "[제안 제목 2]" | [내용 요약] | [섹션명] |

**분리 후 처리:**
- 원본 문서: 분리된 내용 제거 + 새 문서 링크 추가
- 신규 문서: 원본 문서 속성 복사 + 백링크 추가

⚠️ **주의:** 문서 분리 시 신규 Notion 페이지가 생성됩니다.
```
